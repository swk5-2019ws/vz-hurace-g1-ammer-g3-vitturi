...
namespace Hurace.Core.Mapper
{
    internal class AttributeParser
    {
        ...
        {
            return propertyInfo.GetCustomAttributesData().Count(attribute => attribute.AttributeType == typeof(ForeignKey)) == 1;
        }
        ...
        public static bool GetKeyIsGenerated(PropertyInfo propertyInfo)
        {
            ...
            var keyAttributes = propertyInfo.GetCustomAttributesData().Where(attribute => attribute.AttributeType == typeof(Key)).ToArray();
        ...
        /// <summary>
        /// Returns the value of the Column or ForeignKey attribute of a property info. If no attribute is found the property name is used.
        /// </summary>
        /// <param name="propertyInfo">The property info</param>
        ...
        public static string GetColumnName(PropertyInfo propertyInfo)
        {
            ...
            var columnAttributes = propertyInfo.GetCustomAttributesData().Where(attribute => attribute.AttributeType == typeof(Column)).ToArray();
            if (columnAttributes.Length == 1)
                name = (string) columnAttributes[0].ConstructorArguments[0].Value;

            var foreignKeyAttributes = propertyInfo.GetCustomAttributesData().Where(attribute => attribute.AttributeType == typeof(ForeignKey)).ToArray();
        ...
        {
            return GetAllProperties(type).Where(pi => pi.PropertyType.IsGenericType && pi.PropertyType.GetGenericTypeDefinition() == typeof(ICollection<>));
        }
    }
}